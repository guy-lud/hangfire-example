...
[assembly:InternalsVisibleTo("Hangfire.Postgres.RabbitMq.Tests")]

namespace Hangfire.Postgres.RabbitMq
{
    /// <summary>
    /// An <see cref="IConnection"/> implementatation for RabbitMQ.
    /// Maintains a single RabbitMQ <see cref="IModel"/>, as well as a dedicated <see cref="QueueingBasicConsumer"/> (aka channel) for publishing and 
    /// one for consuming messages. The consumer channel owns a <see cref="IPersistentJobQueueProvider"/> for each
    /// queue configured on the parent <see cref="IPersistentJobQueue"/>.
    /// </summary>
                ...
                () => {
                    try
                    {
                        lock (ConsumerLock)
                        {
                            ...
                            _consumerChannel.Close(RabbitMQ.Client.Framing.Constants.ReplySuccess, "Requeue");
                        }
                    }
                    catch (Exception ex)
                    {
